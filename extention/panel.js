...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  try {
    chrome.storage.local.get(['queueAutoclearHours'], r => {
...
async function processQueue(){
  // Pick the next waiting item. Done/error items stay in the array.
  const idx = queue.findIndex(q => q.status === 'waiting');
  if(idx < 0){ queueProcessing=false; updateQueueUI(); return; }
  ...
  es.addEventListener('progress',e=>{
    ...
    item.progress = p;
    $('progfill').style.width=Math.round(p)+'%';
    $('proglbl').textContent=Math.round(p)+'%';
  ...
  es.addEventListener('done',async e=>{
    ...
    // Item is already marked done above — leave it in the queue. Just
    // re-render to refresh the "Load again" action button and look for
    // more waiting items to process.
    updateQueueUI();
    setTimeout(()=>$('progwrap').classList.add('hide'),2000);

    // Immediately refresh the local history view so the new track
    // appears at the top WITHOUT waiting for the 4-second polling
    // tick. The poll is still running as a safety net for downloads
    // happening in OTHER places (desktop app, another panel) — but
    // for THIS panel's own download we don't need to wait. The pulse
    // animation in renderHist will draw the eye to the new row.
    if(backendOnline){
      loadHist();
    }

    if(queue.some(q => q.status === 'waiting')){
  ...
  es.addEventListener('error',e=>{
    ...
    if(queue.some(q => q.status === 'waiting')) setTimeout(processQueue,1000);
    else{ queueProcessing=false; updateQueueUI(); }
  });
  ...
  es.onerror=()=>{
    ...
    updateQueueUI();
    $('progwrap').classList.add('hide');
    if(queue.some(q => q.status === 'waiting')) setTimeout(processQueue,2000);
    else{ queueProcessing=false; updateQueueUI(); }
  };
}
